  ...
  return (
    <div className="p-8">
      <div className="space-y-8 max-w-2xl">
        <div>
          <h1 className="text-3xl font-bold text-ink mb-2">Authentification à deux facteurs</h1>
          <p className="text-muted-foreground">Renforcez la sécurité de votre compte administrateur avec Google Authenticator.</p>
        </div>
        ...
        <div className="cyna-card p-6 flex items-center justify-between">
          ...
          <span className={`px-3 py-1 rounded-full text-xs font-bold ${twoFaEnabled ? 'bg-[#10B981]/12 text-success border border-[#10B981]/35' : 'bg-bg-muted text-muted-foreground border border-border'}`}>
        ...
        {step === 'idle' && (
          <div className="cyna-card p-6">
            {!twoFaEnabled ? (
              ...
              <div>
                <h2 className="text-xl font-bold text-ink mb-3">Désactiver la 2FA</h2>
                <p className="text-muted-foreground text-sm mb-6">
        ...
        {step === 'setup' && (
          <div className="cyna-card p-6 space-y-6">
            <div>
              ...
              <p className="text-muted-foreground text-sm mb-4">Ouvrez Google Authenticator → + → Scanner un QR code</p>
              {qrCode && (
            ...
            <div>
              <h2 className="text-xl font-bold text-ink mb-2">Ou entrez la clé manuellement</h2>
              <div className="flex items-center gap-3 bg-bg-subtle border border-border rounded-xl px-4 py-3">
                ...
                <code className="text-primary font-mono text-sm tracking-widest break-all">{secretKey}</code>
              </div>
              <p className="text-muted-foreground text-xs mt-2">Émetteur : CYNA · Type : TOTP · Intervalle : 30s</p>
            </div>

            <div>
              <h2 className="text-xl font-bold text-ink mb-2">2. Confirmez avec le premier code</h2>
              <p className="text-muted-foreground text-sm mb-4">Entrez le code à 6 chiffres affiché dans l'application pour terminer la configuration.</p>
        ...
        {step === 'disable' && (
          <div className="bg-destructive/10 border border-destructive/30 rounded-2xl p-6 space-y-4">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-xl font-bold text-ink">Désactiver la 2FA</h2>
            </div>
            ...
            <div className="flex gap-3 flex-wrap">
              <input
                ...
                value={code} onChange={e => setCode(e.target.value.replace(/\D/g, ''))}
                placeholder="000000"
                className="w-40 bg-card border border-destructive/30 rounded-xl px-4 py-3 text-ink text-center text-xl font-mono tracking-widest focus:outline-none focus:ring-4 focus:ring-[#EF4444]/15 focus:border-[#EF4444]"
              />
        ...
        <div className="bg-[#00B4D8]/6 border border-[#00B4D8]/20 rounded-2xl p-5 text-sm text-muted-foreground space-y-1">
          <p className="font-semibold text-primary mb-2">Comment ça fonctionne</p>
          <p>• La 2FA ajoute une couche de sécurité : à chaque connexion admin, un code temporaire vous sera demandé.</p>
          <p>• Le code change toutes les <strong className="text-ink">30 secondes</strong> et ne peut être utilisé qu'une seule fois.</p>
          <p>• Applications compatibles : <strong className="text-ink">Google Authenticator</strong>, Authy, 1Password, Bitwarden.</p>
