  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-8 flex-wrap gap-4">
        <div>
          <h1 className="text-3xl font-bold text-ink mb-1">Commandes</h1>
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {orders.length === 0 ? (
            <div className="text-center py-16 cyna-card text-muted-foreground">Aucune commande</div>
          ) : orders.map(order => {
            ...
            return (
              <div key={order.id} className="cyna-card overflow-hidden">
                <div className="flex items-center gap-4 p-5 flex-wrap">
                  ...
                  <span className="text-muted-foreground text-xs">{new Date(order.created_at).toLocaleDateString('fr-FR')}</span>
                </div>
                {isOpen && (
                  <div className="border-t border-border px-5 py-4 space-y-2 bg-bg-subtle">
