    ...
    <div className="min-h-screen bg-card py-12">
      <div className="max-w-3xl mx-auto px-6">
        <div className="text-center mb-12 fade-up">
          ...
          <h1 className="text-4xl font-bold text-success mb-3">Commande confirmée !</h1>
        ...
        <div className="cyna-card overflow-hidden mb-8">
          <div className="bg-bg-subtle p-6 border-b border-border">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-2xl font-bold text-ink">Récapitulatif de commande</h2>
            </div>
          </div>
            ...
            {cart.length > 0 ? cart.map(item => (
              <div key={`${item.id}-${item.duration}`}
                ...
                <div>
                  ...
                  <div className="text-sm text-muted-foreground">Quantité : {item.quantity} • {item.duration === 'monthly' ? 'Mensuel' : 'Annuel'}</div>
                </div>
                <div className="text-right">
                  <div className="text-lg font-bold text-ink">{(item.price * item.quantity).toLocaleString('fr-FR')}€</div>
                  <div className="text-sm text-muted-foreground">{item.price.toLocaleString('fr-FR')}€ × {item.quantity}</div>
                </div>
              </div>
            )) : (
              <p className="text-muted-foreground text-center py-4">Commande enregistrée</p>
            )}
            {total > 0 && (
              <div className="pt-4 flex items-center justify-between">
                <span className="text-xl text-ink font-semibold">Total mensuel</span>
                <span className="text-3xl font-bold text-ink">{total.toLocaleString('fr-FR')}€</span>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="bg-[#10B981]/8 border border-[#10B981]/30 rounded-2xl p-6 mb-8">
          <h3 className="text-success font-semibold mb-3">🚀 Prochaines étapes</h3>
